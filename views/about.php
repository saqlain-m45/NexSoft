...
<section class="page-header">
    <div class="container">
        ...
        <p class="page-header-subtitle reveal">The story behind the agency transforming businesses through technology.
        </p>
    </div>
</section>
    ...
    <div class="container">
        <div class="row align-items-center g-5">
            <div class="col-lg-6 reveal">
                ...
                <p
                    style="color:var(--text-muted); font-weight:300; font-size:1.05rem; line-height:1.85; margin-bottom:1.5rem;">
                    NexSoft Hub is a full-service software consulting agency founded by a team of passionate developers,
                    designers, and strategists who believe technology should be accessible, powerful, and beautifully
                    crafted for every business.
                </p>
                <p
                    style="color:var(--text-muted); font-weight:300; font-size:1rem; line-height:1.85; margin-bottom:2rem;">
                    Since our founding, we have partnered with startups, SMEs, and enterprises across 30+ industries —
                    helping them launch faster, scale smarter, and stand out in an increasingly digital world. Every
                    line of code we write, every pixel we design, and every strategy we craft is done with one goal:
...
foreach ($stats as $s): ?>
                    <div
                        style="background:var(--bg-alt);border:1px solid var(--border);border-radius:var(--radius);padding:2rem 1.5rem;text-align:center;">
                        <i class="bi <?php echo $s['icon']; ?>"
                            style="font-size:2rem;color:var(--secondary);margin-bottom:0.75rem;display:block;"></i>
                        <div style="font-size:2rem;font-weight:900;color:var(--primary);">
                            <?php echo $s['num']; ?>
                        </div>
                        <div
                            style="font-size:0.75rem;text-transform:uppercase;letter-spacing:1px;color:var(--text-muted);margin-top:4px;font-weight:600;">
                            <?php echo $s['label']; ?>
                        </div>
                    </div>
                    <?php
...
foreach ($mvs as $mv): ?>
            <div class="col-lg-6 reveal">
                <div class="about-mission-card">
                    <h4><i class="bi <?php echo $mv['icon']; ?> me-2"></i>
                        <?php echo $mv['label']; ?>
                    </h4>
                    <h3>
                        <?php echo $mv['title']; ?>
...
<section class="team-tree-section">
    <div class="container">
        <div class="row justify-content-center text-center mb-5">
            <div class="col-lg-6 reveal">
                ...
                <p class="section-subtitle mx-auto">Talented specialists united by a shared passion for building
        ...
        <div class="text-center py-5 reveal">
            <div
                style="width:80px;height:80px;background:rgba(14,165,164,0.08);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 1rem;border:2px dashed rgba(14,165,164,0.25);">
            ...
            <p style="color:var(--text-muted);font-size:0.9rem;">Our team directory is being set up. Check back soon!
            </p>
        </div>
    ...
    $rows = [];
    // Row 1: Founder (Member 1)
    if (count($teamMembers) >= 1) {
        $rows[] = [$teamMembers[0]];
    }
    // Row 2: Co-founder (Member 2)
    if (count($teamMembers) >= 2) {
        $rows[] = [$teamMembers[1]];
    }
    // Row 3+: Others
    if (count($teamMembers) > 2) {
        $others = array_slice($teamMembers, 2);
        foreach (array_chunk($others, 4) as $chunk) {
            $rows[] = $chunk;
        }
    }
?>
...
?>
                <div class="org-node">
                    <div class="org-avatar-wrap" <?php if (!empty($member['bio'])): ?>title="
                        <?php echo htmlspecialchars($member['bio']); ?>"
                        <?php
            endif; ?>>
                        <?php if (!empty($member['photo'])): ?>
                        <img src="<?php echo baseUrl('assets/uploads/team/' . htmlspecialchars($member['photo'])); ?>"
                            alt="<?php echo htmlspecialchars($member['name']); ?>" class="org-avatar-img"
                            onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">
                        <div class="org-avatar-initials" style="display:none;">
                            <?php echo $initials; ?>
                        </div>
                        <?php
            else: ?>
...
foreach ($story as $i => $s): ?>
                    <div style="display:flex;gap:2rem;margin-bottom:2.5rem;align-items:flex-start;">
                        <div style="flex-shrink:0;text-align:center;">
                            <div
                                style="width:56px;height:56px;background:linear-gradient(135deg,var(--secondary),var(--secondary-dark));border-radius:50%;display:flex;align-items:center;justify-content:center;color:white;font-weight:700;font-size:0.75rem;">
                                <?php echo $s['year']; ?>
                            </div>
                            <?php if ($i < count($story) - 1): ?>
                            <div style="width:2px;height:40px;background:var(--border);margin:8px auto;"></div>
                            <?php
    endif; ?>
                        </div>
                        <div
                            style="background:var(--white);border:1px solid var(--border);border-radius:var(--radius);padding:1.5rem;flex:1;">
                            <h4 style="font-size:1.1rem;color:var(--primary);margin-bottom:0.5rem;">
                                <?php echo $s['title']; ?>
                            </h4>
                            <p style="font-size:0.9rem;color:var(--text-muted);line-height:1.75;margin:0;">
                                <?php echo $s['text']; ?>
                            </p>
                        </div>
                    </div>
